...
namespace NFLGameWeather.Model.Services
{
    ...
    {
        public async Task<Forecast> GetForecastAsync(Stadium stadium)
        {
            using (HttpClient client = new HttpClient())
            {
                string apiKey = "MSTYhksuum4CfZgJFaqyHXPukFGrr4Ya";
                string path = $"http://dataservice.accuweather.com/locations/v1/cities/geoposition/search?apikey={apiKey}&q={stadium.GeoLatitude.ToString(CultureInfo.InvariantCulture.NumberFormat)},{stadium.GeoLongitude.ToString(CultureInfo.InvariantCulture.NumberFormat)}";
